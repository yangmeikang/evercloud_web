...
def keystoneclient(request=None, admin=False):
    ...
    cacert = getattr(settings, 'OPENSTACK_SSL_CACERT', None)
    conn = api_version['client'].Client(auth_url=request.get("auth_url"),
...
def tenant_create(request, name, description="create by evercloud api", enabled=True,
                  domain=None, **kwargs):
